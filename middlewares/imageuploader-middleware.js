    ...
    let upload = multer({
        ...
        fileFilter: function (req, file, cb) {
            ...
            if (!file.originalname.match(/\.(jpg|jpeg|png|gif)$/)) {
                return cb(errors.getError('ESS42212'), false);
            }
...
const uploadVideoMedia = (media, option) => {
    ...
    let storage = multer.diskStorage({
        ...
        filename: (req, file, cb) => {
            ...
            if (overridefilename) {
                req.body.user_short = fileStorageName + ext 
                cb(null, (!_.isEmpty(fileStorageName)) ? fileStorageName + ext : file.originalname)
            } else {
                cb(null, (fileStorageName || name) + '-' + (new Date()).getTime() + ext)
            }
...
const uploadUserVideo = (upFileName, upconfig) => {
    return uploadVideoMedia(upFileName, {
        fileName: (req) => {
            return `${upconfig.customFileName}`+Date.now()
        },
        destination: (req, cb) => {
